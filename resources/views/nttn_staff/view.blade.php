                ...
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4 mx-5">
                        <div class="flex justify-center items-center">
                            <p class="font-bold text-gray-600 text-lg">NTTN Staff Details</p>
                        </div>
                        <div class="flex justify-center items-center gap-3">
                            <a href="{{ route('nttn_staff.edit', $staff->id) }}" class="select-none bg-red-500 hover:bg-red-700 rounded shadow-md flex justify-center items-center flex-col gap-2 transition-shadow hover:shadow-lg p-2 px-6">
                                <span class="font-bold text-white">Edit</span>
                            </a>
                            <a href="{{ route('nttn_staff.index') }}" class="select-none bg-[#2B7381] hover:bg-[#1e5661] rounded shadow-md flex justify-center items-center flex-col gap-2 transition-shadow hover:shadow-lg p-2 px-6">
                    ...
                    <div class="mt-6 bg-gray-100 rounded py-5 mx-5 px-6 flex flex-col justify-center items-start gap-1">
                        ...
                        <div class="flex justify-between items-center">
                            <p class="font-bold text-gray-600">Division: <span class="font-normal">{{ $staff->division?->name }}</span></p>
                        </div>
                        <div class="flex justify-between items-center">
                            <p class="font-bold text-gray-600">District: <span class="font-normal">{{ $staff->district?->name }}</span></p>
                        </div>
                        <div class="flex justify-between items-center">
                            <p class="font-bold text-gray-600">Upazila: <span class="font-normal">{{ $staff->upazila?->name }}</span></p>
                        </div>
                        <div class="flex justify-between items-center">
                            <p class="font-bold text-gray-600">Union: <span class="font-normal">{{ $staff->union?->name }}</span></p>
                        </div>
                        @if ($myRole == 'Super Admin')
                            <div class="flex justify-between items-center">
                                <p class="font-bold text-gray-600">NTTN Provider: <span class="font-normal">{{ getNttnProvider($staff)->name }}</span></p>
                            </div>
                        @endif
                    </div>
                    <div class="mt-6 bg-gray-100 rounded py-5 mx-5 px-6 flex flex-col justify-center items-start gap-1">
                        <div class="flex justify-between items-center">
                            <p class="font-bold text-gray-600">Status: <span class="font-normal">{{ ucfirst($staff->status == 1 ? 'Active' : 'Inactive') }}</span></p>
                        </div>
                        <div class="flex justify-between items-center">
                    ...
                    <div class="mt-6 bg-gray-100 rounded py-5 mx-5 px-6 flex flex-col justify-center items-start gap-1">
                        ...
                        <div class="flex justify-between items-center gap-2 mt-2">
                            <form method="POST" action="{{ route('change.password', $staff->id) }}" class="space-y-3">
                                @csrf
                                <div>
