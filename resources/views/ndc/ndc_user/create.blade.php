                    ...
                    <form action="{{ route('ndc.user.store') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-select id="user_type" name="user_type" class="mt-1 block w-full" required>
                                ...
                                <option @selected(in_array(old('user_type'), ['ndc_admin'])) value="ndc_admin">NDC Admin</option>
                                <option @selected(in_array(old('user_type'), ['ndc_security_admin'])) value="ndc_security_admin">NDC Security Admin</option>
                                <optgroup label="Visitor">
                                    <option @selected(in_array(old('user_type'), ['ndc_internal'])) value="ndc_internal">Internal</option>
                                    <option @selected(in_array(old('user_type'), ['ndc_customer'])) value="ndc_customer">Customer</option>
                                    <option @selected(in_array(old('user_type'), ['ndc_vendor'])) value="ndc_vendor">Vendor</option>
                                </optgroup>
                            </x-select>
                            <x-input-error class="mt-2" :messages="$errors->get('user_type')" />
                        </div>

                        <div id="sector_container" class="{{ old('ndc_admin_sector') ? 'block' : 'hidden' }}">
                            <x-input-label for="ndc_admin_sector" :value="__('Sector')" />
                            <x-select id="ndc_admin_sector" name="ndc_admin_sector" class="mt-1 block w-full" :disabled="old('ndc_admin_sector') ? false : true">
