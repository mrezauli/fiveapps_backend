            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center">
                        <h1 class="text-lg font-semibold">List of district</h1>
                        <a href="{{ route('regions.district.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold text-sm py-2 px-4 rounded">Add new district</a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($districts as $district)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $district->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $district->bn_name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $district->division->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium flex justify-start items-center gap-2">
                                            <a href="{{ route('regions.district.edit', $district->id) }}" class="text-indigo-600 hover:text-indigo-900 hover:underline">Edit</a>
                                            <a href="{{ route('regions.district.delete', $district->id) }}" class="delete-lnk text-red-600 hover:text-red-900 hover:underline">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    @section('scripts')
        <script>
            ...
            $('a.delete-lnk').confirm({
                ...
                content: "Do you really want to delete this district?",
                buttons: {
                    no: {
                        text: 'No',
