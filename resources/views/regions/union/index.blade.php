            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center">
                        <h1 class="text-lg font-semibold">List of union</h1>
                        <a href="{{ route('regions.union.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold text-sm py-2 px-4 rounded">Add new union</a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($unions as $union)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $union->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $union->bn_name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $union->upazila->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $union->upazila->district->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 text-gray-900">{{ $union->upazila->district->division->name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium flex justify-start items-center gap-2">
                                            <a href="{{ route('regions.union.edit', $union->id) }}" class="text-indigo-600 hover:text-indigo-900 hover:underline">Edit</a>
                                            <a href="{{ route('regions.union.delete', $union->id) }}" class="delete-lnk text-red-600 hover:text-red-900 hover:underline">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    @section('scripts')
        <script>
            ...
            $('a.delete-lnk').confirm({
                title: "Delete Union",
                content: "Do you really want to delete this union?",
                buttons: {
                    no: {
                        text: 'No',
