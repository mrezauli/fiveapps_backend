        ...
        @foreach ($examRegs as $examReg)
            <div class="card card-item card-item-list-layout">
                <div class="card-image">
                    <a href="{{ route('examinee.payment.hosted.checkout', $examReg->id) }}" class="d-block">
                ...
                <div class="card-body">
                    <h5 class="card-title"><a
                            href="{{ route('examinee.payment.hosted.checkout', $examReg->id) }}">{{ $examReg->examType->name }}</a>
                    </h5>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="pl-3 card-action-wrap">
                            <a href="{{ route('examinee.payment.hosted.checkout', $examReg->id) }}"
                                class="ml-1 shadow-sm cursor-pointer icon-element icon-element-sm text-success"
                                data-toggle="tooltip" data-placement="top" data-title="Pay"><i class="la la-edit"></i></a>
                        </div>
                    </div>
                </div><!-- end card-body -->
            </div><!-- end card -->
