<x-app-layout>
  @section('title', 'Edit ITEE Exam Type')
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      {{ __('Edit ITEE Exam Type') }}
    </h2>
  </x-slot>
      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="flex justify-between items-center mb-4 mx-5 pt-6">
          <div class="flex justify-center items-center">
            <p class="font-bold text-gray-600 text-lg">Edit ITEE Exam Type</p>
          </div>
          <a href="{{ route('itee.exam.type.index') }}"
            class="select-none bg-[#2B7381] hover:bg-[#1e5661] rounded shadow-md flex justify-center items-center flex-col gap-2 transition-shadow hover:shadow-lg p-2 px-6">
        ...
        <div class="p-6 pt-0 text-gray-900 max-w-xl">
          <form action="{{ route('itee.exam.type.store') }}" enctype="multipart/form-data" method="POST" class="space-y-4">
            ...
            <div>
              ...
              <x-text-input id="image" name="image" type="file" accept="image/png, image/jpeg"
                class="mt-1 block w-full p-2 border" :value="old('image')" required autocomplete="image" />
              <x-input-error class="mt-2" :messages="$errors->get('image')" />
            </div>

            @if ($item->image)
              <div class="p-1 rounded overflow-hidden border">
                <img src="{{ asset($item->image) }}" alt="{{ $item->name }}" class="w-full">
              </div>
            @endif
