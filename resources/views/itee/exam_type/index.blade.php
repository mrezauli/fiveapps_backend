            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center">
                        <h1 class="text-lg font-semibold">List of Exam Type</h1>
                        <a href="{{ route('itee.exam.type.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold text-sm py-2 px-4 rounded">Add new Exam Type</a>
                    </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($items as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                            <div class="text-sm leading-5 font-medium text-gray-900"> {{ $item->category?->name ?? 'None' }} </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-no-wrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium flex justify-end items-center gap-2">
                                            <a href="{{ route('itee.exam.type.edit', ['id' => $item->id]) }}" class="text-indigo-600 hover:text-indigo-900 hover:underline">Edit</a>
                                            <a href="{{ route('itee.exam.type.delete', ['id' => $item->id]) }}" class="delete-bcc-staff text-red-600 hover:text-red-900 hover:underline">Delete</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    @section('scripts')
        <script>
            ...
            $('a.delete-bcc-staff').confirm({
                ...
                content: "Do you really want to delete this Exam Type?",
                buttons: {
                    no: {
                        text: 'No',
