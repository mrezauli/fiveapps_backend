<x-app-layout>
    @section('title', 'Create new ITEE Course Outline')
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Create new ITEE Course Outline') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 max-w-xl">
                    <form action="{{ route('itee.course.outline.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div>
                            <x-input-label for="course_outline_file" :value="__('Syllabus File')" :exception="__('| (PDF)')" />
                            <x-text-input id="course_outline_file" name="course_outline_file" type="file" class="mt-1 block w-full" accept="application/pdf" required autocomplete="course_outline_file" />
