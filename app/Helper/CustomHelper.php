...
class CustomHelper
{
    ...
    {
        if (in_array(\App\Helper\DateTimeHelper::dayFormatter1($date), Attendance::$holidayArray)) {
            return true;
        }
        return false;
    }
     ...
     * @throws \Exception
     */
    public static function fillAsDateArray($start_date, $end_date)
    {
        $date_range = [];
    ...
    {
        $roleName = CustomHelper::userRoleName();
        if ($roleName == 'Institute Head') {
            $students = User::where('institute_id', auth()->user()?->institute_id)->whereHas('roles', function ($query) {
                $query->where('name', 'Student');
            })->count();
            $limited = auth()->user()->studentLimit?->limit ?? 105;
            $limit = $limited - $students;
            return $limit;
        } else {
            return 'nothing';
        }
